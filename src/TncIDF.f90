function tncidf( p, q, a1, delta, maxitr, ier )
...
!*********************************************************************************************!
    ...
    !  ------------------
    real(kind=8) :: k
    real(kind=8), external :: tnccdf, dtrinv

    k = sqrt(q/2) * Exp(dlgama((q-1)/2.) - dlgama(q/2.) ) 

    tncidf = dtrinv( func, p,                             &
                .FALSE., .FALSE., -1.0e6_8, 1.0e6_8,      &
                a1 * k,                                   &
                sqrt( q*(1+a1**2)/(q-2) - a1**2 * k**2 ), &
                +1.0e-6_8, 1000,                          &
                ier                                       &
            )
...
contains
    function func(x, iok)
        ...
        real(kind=8) :: func
        func = tnccdf(x, q, a1, delta, maxitr, iok)
    end function func
